...
$(document).ready(function(){
    $(".breadcrumb").empty();
    var breadcrumb = '<li class="selected"><a class="" alt="faceStatistics.html" id="5928f634d4e0c3a0a7c4659e">人脸识别使用详情</a></li><li><a class="" alt="faceAbnormalStatistics.html" id="">人脸识别异常详情</a></li>';
...
function line(time,success,total){
    // 路径配置
    require.config({
        paths: {
...
function pullModel(start_Time,end_Time,page){
    ...
    $.ajax({
        url:commUrl+'/member/userStatistics',
        //type:'POST',
        cache:false,
        data:obj,
        dataType:'json',
        success:function(data){
            ...
            var html =  '<tr class="grid-head-th">'+
                ...
                '<th class="successNum" style="width: 152px; ">新增用户</th>'+
                '<th class="delateNum" style="width: 152px; ">活跃用户</th></tr>';
            var page = '';
            if(data.code == '1'){
                if(data.data.timeList.length !== 0){
                    ...
                    $.each(data.data.timeList,function(i,key){
                       ...
                       // _error.push(key.errorCount);
                        _success.push(key.newly);
                        _total.push(key.active);
                ...
                if(data.data.survey.length == 0){
                    ...
                    $.each(data.data.survey,function(i,key){
                        ...
                        html += '<tr class="grid-body-tr" > ' +//<td class="start_Time"  style="width:200px">'+key.date+'</td>
                            ...
                            '<td class="successNum"  style="width:152px;">'+key.newly+'</td> ' +
                            '<td class="delateNum"  style="width:152px;">'+key.active+'</td></tr>'
                    })
                }
            }
            $(".pages ul").html(page);
