            ...
            if(data.code == '1'){
                $.each(data.data,function(i,key){
                    ...
                    html += '<div class="grid-body-tr" > <div class="number"  style="width:150px">'+sort+'</div>' +
                        '<div class="title"  style="width:390px;max-width:300px; ">'+key.className+'</div>' +
                        '<div class="operation"  style="width:410px">' +
                        '<a href="javascript:void(0);" onclick="" class="addClass" data-id="'+key.id+'" title="'+key.title+'"  data-name="'+key.className+'">编辑</a>' +
...
function deleted(){
    $('.deleteQuestion').on('click',function(){
        ...
        $.ajax({
            url:commUrl+'/teacher/delClassGroup',
            data:{
                token:$.cookie("token"),
                id:_id
            },
...
function addClassConfirm() {
    ...
    $('.addClassConfirm').on('click',function(){
        ...
        if(id == undefined || id == ''){
            var _url = commUrl+'/teacher/addClassGroup'
        }else{
            var _url = commUrl+'/teacher/updateClassGroup'
            obj.id = id;
