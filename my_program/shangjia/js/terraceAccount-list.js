...
$(document).ready(function(){
    $(".breadcrumb").empty();
    var breadcrumb = '<li class="selected"><a class="" alt="terraceAccount-list.html" id="58cb95033c9e9c3c425683b6">子账号列表</a></li>';
                ...
                if(response.data.datas == ''){
                    ...
                    $.each(response.data.datas,function(i,key){
                        ...
                        html += '<div class="grid-body-tr"> ' +
                            ...
                            '<div class=""><span title="">'+key.name+'</span></div>'+
                            '<div class=""><span title="" data-id="'+key.classId+'">'+key.className+'</span></div>'+
                             '<div class=""><span title="">'+key.phone+'</span></div>' ;
...
function deleted(){
    $(".deleted").on('click',function(){
        //alert($(this).attr('id'));
        $.ajax({
            url: commUrl+'/teacher/delTeacher',
            type:'POST',
            data:{
                token:$.cookie("token"),
                id:$(this).attr('id')
