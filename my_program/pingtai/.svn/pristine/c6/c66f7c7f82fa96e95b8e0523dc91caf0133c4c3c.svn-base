...
$(document).ready(function(){
    $(".breadcrumb").empty();
    var breadcrumb = '<li><a class="" alt="voiceStatistics.html" id="5928f634d4e0c3a0a7c4659e">语音识别使用详情</a></li><li class="selected"><a class="" alt="voiceAbnormalStatistics.html" id="">语音识别异常详情</a></li>';
...
function pullModel(start_Time,end_Time,page){
    ...
    if(shopId == undefined)
    {
        ...
        $.ajax({
            url:commUrl+'/speech/speechAbnormalSurvey',
            //type:'POST',
            cache:false,
            data:obj,
            dataType:'json',
                    ...
                    if(data.data == ''){
                        ...
                        $.each(data.data,function(i,key){
                            ...
                            html += '<tr class="grid-body-tr" > ' +//<td class="start_Time"  style="width:200px">'+key.date+'</td>
                                '<td class="totalNum"  style="width:152px;">'+key.business+'</td> ' +
                                '<td class="successNum"  style="width:152px;">'+(key.consumingTime / 1000)+'秒</td> ' +
        ...
        //跑起来
        $.ajax({
            url:commUrl+'/speech/speechAbnormalDetail',
            //type:'POST',
            cache:false,
            data:obj,
            dataType:'json',
            success:function(data){
                ...
                $(".grid-body").empty();
                var html =  '<tr class="grid-head-th">'+
                    '<th class="start_Time" style="width: 130px;">请求时间时间</th>'+
                    /*'<th class="totalNum" style="width: 152px;">请求照片</th>'+*/
                    '<th class="outNum" style="width: 202px; ">机器人</th>'+
                    '<th class="successNum" style="width: 152px; ">总耗时</th>'+
                    '<th class="perc" style="width: 202px; ">异常原因</th>'+
                    '<th class="delateNum" style="width: 264px; ">处理详情</th>'+
                    '</tr>';
                var page = '';
                if(data.code == '1'){
                    if(data.data.datas == ''){
                        ...
                        $.each(data.data.datas,function(i,key){
                            ...
                            html += '<tr class="grid-body-tr" ><td class="start_Time"  style="width:130px">'+key.requestTime+'</td>' +//
                                /*'<td class="successNum"  style="width:152px;"><img src="'+key.photoUrl+'" style="width:120px; height:80px" alt=""/></td> ' +*/
                                '<td class="outNum"  style="width:202px;">'+key.maid+'</td>'+
                                '<td class="totalNum"  style="width:152px;">'+(key.consumingTime / 1000)+'</td> ' +
                                '<td class="delateNum"  style="width:202px;">'+key.errorMsg+'</td> ' +
                                '<td class="outNum"  style="width:264px;">前端耗时'+(key.appTime / 1000)+',讯飞耗时'+(key.iflytekTime / 1000)+'</td> </tr>';
                        })
                    }
                    if(data.data.page == '1') {
...
function shop(){
    $('#shop').on('change',function(){
        var _value = $(this).children('option:selected').attr('id');
        //console.log(e.id);
        findRobot(_value)
        listPage();
    })
