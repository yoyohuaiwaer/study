$(document).ready(function() {
	...
	addimage('.confirmVideo');
	overshow();//弹出新增
	cancelAdd();//取消新增题
	addDirection();//新增题的提交按钮
	deleDirection();//删除某一道题
	chooseMultimedia();//上传视频oss
})
		...
		success: function(data) {
			...
			if(data.code == '1') {
				...
				if(data.type=='image'){//图片类型
					...
					$('#type').children('option:eq(3)').prop('selected',true);
					type = '#videoUpload';
					$("#vedioTitle").val(data.title);
					$("#videoId").attr({src:data.video}).show();
					/*$("#url").val(data.url);
					if(data.img !=="")
					{
						$('#imageId').attr('src', data.img);
					...
					$('.confirmVideo').attr({'data-id':data.id,'data-meid':data.meid});
				}
				//editDirection();
				console.log(type);
				changeShow(type);
...
function uploadImage(){
	$(".changeImg .file_upload").change(function() {
