...
function getTodayModel() {
	$.ajax({
		...
		success: function(data) {
			...
			console.log(data)
			if(data.code == '1') {
				$('.duration').html(formatDuring(data.data.duration));
				$('.callRate').html(((data.data.callRate)*100).toFixed(2)+"%");
				$('.connectRate').html(((data.data.connectRate)*100).toFixed(2)+"%");
				$('.problemNumber').html(data.data.problemNumber);
				$('.accuracy').html(((data.data.accuracy)*100).toFixed(2)+"%");
			}
		}
	})
}
		...
		success: function(data) {
			...
			if(data.code == '1') {
				...
				$.each(data.data.datas, function(i, key) {
					console.log(((key.connectRate)*100).toFixed(2))
					sort = i + 1;
					html += '<tr class="grid-body-tr" > <td class="start_Time">' + key.date + '</td> ' +
						'<td class="totalNum">' + formatDuring(key.duration) + '</td> ' +
						'<td class="totalNum">' + ((key.callRate)*100).toFixed(2) + '%</td> ' +
						'<td class="totalNum">' + ((key.connectRate)*100).toFixed(2) + '%</td> ' +
						'<td class="totalNum">' + key.problemNumber + '</td> ' +
						'<td class="noLoginNum">' + ((key.accuracy)*100).toFixed(2) + '%</td> </tr>'
				})
				if(data.data.page == '1') {
