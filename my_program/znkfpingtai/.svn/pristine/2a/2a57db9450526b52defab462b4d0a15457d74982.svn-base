...
//var commUrl = 'http://172.16.8.71:8081/manage';//胡威
...
function findMenus(parentId){
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.code == 1) {
                var breadcrumb1 = '';
                breadcrumb1 = '<li class="selected"><a alt="'+menuUrl+''+data.data[0].url+'" id="'+data.data[0].id+'">官方行业问答列表</a></li>';
...
function searchbtn(){
    $('.searchbtn').on('click',function(){
        $.cookie('rel1', $(this).attr('rel'),{path:'/'});
        pagenation();
...
function pagenation(page){
    ...
    if($.cookie('rel1') == 0){
        ...
        if($('#search').children('option:selected').val() == 'question') {
            key = $('.searchText').val();
        }else {
            ans = $('.searchText').val();
        }
