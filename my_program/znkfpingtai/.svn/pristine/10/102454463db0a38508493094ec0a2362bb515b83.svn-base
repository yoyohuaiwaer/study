...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        data: {
            token:$.cookie("token"),
            parentId:'5850c20bb2a99afa74c513b4'
        },
        success: function(data){
            ...
            if(data.code == 1) {
                //$(".breadcrumb").empty();
                var breadcrumb = '';
                breadcrumb = '<li class="selected"><a alt="'+menuUrl+''+data.data[0].url+'" id="'+data.data[0].id+'">'+data.data[0].name+'</a></li>';
...
function loadentities(){
    var obj = {}
    if($('.detailConfirm').attr('data-content-id') == 'node1'){
        obj = {token: $.cookie('token'),
            industry:$('#industry').children('option:selected').attr('value')}
    }else {
        obj = {token: $.cookie('token'),
            industry:$('#node1').attr('data-industry')
        }
    }
    $.ajax({
        ...
        success: function(response){
          //console.log(response);
            ...
            if(response.code == 1){
                $('#entity').empty();
                var html = '';
                html+='<option value="" id="" >未关联实体</option>'
...
function addElement(){
    $('.treeview').on('click','.addElement',function(){
        var industryName = $('#node1').attr('data-industry');
        var nodeID = '#'+$(this).attr('data-nodeID');
        var html = '';
        ...
        if(i < 7){
            if($(this).parent('.fontBox').parent('li').children(' ul').length == 0){
                ...
                eLength += '_1'
                html = '<ul data-col="'+i+'"><li class="last" data-col="'+i+'" id="'+eLength+'" rel="1" data-industry="'+industryName+'"> ' ;
                if(i==2){
                    i = '二';
                }else if(i==3){
                    i = '三';
                ...
                html = '<li data-col="'+i+'" id="'+eLength+'" rel="1" data-industry="'+industryName+'">' ;
                if(i==2){
                    i = '二';
                }else if(i==3){
                    i = '三';
...
function addDetails(){
    $('.treeview').on('click','.addDetails',function(){
        ...
        if($(this).parent('.folder').parent('.fontBox').parent('li').attr('rel') == 1){
            ...
            if($(this).parent('.folder').parent('.fontBox').parent('li').attr('id') == 'node1')
                if($(this).parent('.folder').parent('.fontBox').parent('li').attr('data-industry') !== undefined ){
                    var industryName = $(this).parent('.folder').parent('.fontBox').parent('li').attr('data-industry');
                    $('#industry').children('option[value="'+industryName+'"]').attr({selected:true});
                }
            //debugger;
            loadentities();
...
function detailConfirm(){
    $('.detailConfirm').on('click',function(){
        ...
        if($(this).attr('data-content-id') == 'node1'){
            ...
            if($('#question').val() !==''& $('#questions').val()!== ''){
                ...
                var keyWord = $('#questions').val().replace(/\s/gi,'');
                ...
                if($('#answers').val() !== ''){
                    if($('#entity').children('option:selected').attr('id') == ''){
                        ...
                        if(adId == ''){
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-industry':industry,'data-questions':keyWord,'rel':'0','data-entityID':'','data-entityName':'','data-adID':'','data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }else {
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-industry':industry,'data-questions':keyWord,'rel':'0','data-entityID':'','data-entityName':'','data-adID':adId,'data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }

                    }else{
                        if(adId == ''){
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-industry':industry,'data-questions':keyWord,'rel':'0','data-entityID':$('#entity').children('option:selected').attr('id'),'data-entityName':$('#entity').children('option:selected').attr('value'),'data-adID':'','data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }else {
            ...
            if($('#question').val() !==''& $('#questions').val()!== ''){
                ...
                if($('#answers').val() !== ''){
                    if($('#entity').children('option:selected').attr('id') == ''){
                        if(adId == ''){
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-questions':keyWord,'rel':'0','data-entityID':'','data-entityName':'','data-adID':'','data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }else {
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-questions':keyWord,'rel':'0','data-entityID':'','data-entityName':'','data-adID':adId,'data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }

                    }else{
                        if(adId == ''){
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-questions':keyWord,'rel':'0','data-entityID':$('#entity').children('option:selected').attr('id'),'data-entityName':$('#entity').children('option:selected').attr('value'),'data-adID':'','data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }else {
                            $(addId).attr({'data-question':$('#question').val(),'data-answers':$('#answers').val(),'data-questions':keyWord,'rel':'0','data-entityID':$('#entity').children('option:selected').attr('id'),'data-entityName':$('#entity').children('option:selected').attr('value'),'data-adID':adId,'data-sType':$('input[name="sType"]:checked').attr('value')}).children('.fontBox').children('.folder').children('.addDetails').text($('#question').val());
                        }
...
function loadTreeList(){
    if($.cookie('rel') == 1){
        $('#tree').empty().html('<li class="last" data-id="" data-col="1" id="node1" rel="1" data-industry=""><div class="fontBox"><span class="folder"><a class="addDetails">首层问题</a></span><a class="addElement" data-nodeID="node1">+</a></div></li>');
